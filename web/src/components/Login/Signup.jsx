...
const Signup = () => {
  ...
  const signUp = (e) => {
    e.preventDefault();
    if (e.target.password.value.trim() !== e.target.rePassword.value.trim()) {
      alert("Your repassword does not match !");
      return;
    } else {
      dispatch(
        userSignup({
          userName: e.target.userName.value.trim().replace(/\s/g, ""),
          password: e.target.password.value.trim().replace(/\s/g, ""),
        })
      );
    }
  };
    ...
    <SignupStyled>
      ...
      <form className="login-form" onSubmit={(e) => signUp(e)}>
        ...
        <div className="form-error">
          {err && err.map((err, index) => <div key={index}> {err}</div>)}
        </div>
